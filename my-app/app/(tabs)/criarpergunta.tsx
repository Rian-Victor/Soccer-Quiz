...
export default function CreateQuestions() {
  ...
  const loadData = async () => {
    try {
      ...
      const formattedQuestions = data.map((q: any) => {
          ...
          if (q.options && Array.isArray(q.options) && typeof q.correct_option_index === 'number') {
  ...
  const handleSaveNewQuestion = async () => {
    ...
    try {
      ...
      setNewQuestion({ question: "", correctAnswer: "", wrong1: "", wrong2: "", wrong3: "" });
      ...
      <View style={styles.content}>
        ...
        <View style={styles.headerBar}>
          ...
          <TouchableOpacity onPress={() => setModalVisible(true)} disabled={loading}>
            <Image source={require("../../assets/images/plus-square.png")} style={[styles.editIcon, loading && { opacity: 0.5 }]} />
          </TouchableOpacity>
        </View>
          ...
          <View style={styles.questionsContainer}>
            {questions.length === 0 && !loading && (
              <View style={styles.emptyState}>
                <Text style={styles.emptyStateText}>Nenhuma pergunta cadastrada.</Text>
              </View>
            )}
            ...
            {questions.map((item, index) => (
                <View key={item.id} style={styles.questionCard}>
                  ...
                  <Text style={[styles.answerText, {marginTop: 10}]}>{item.correctAnswer}</Text>
                </View>
            ))}
          </View>
        </ScrollView>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                {isTeamQuiz && (
                    <View style={styles.inputContainer}>
                        ...
                        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{flexDirection: 'row', height: 50}}>
                            {teams.map((team) => (
                                <TouchableOpacity 
                                    key={team.id}
                                    style={[
                                ...
                                >
                                    <Text style={[
                                        ...
                                        selectedTeamId === team.id && styles.teamOptionTextSelected
                                    ]}>{team.name}</Text>
                                </TouchableOpacity>
                            ))}
                        </ScrollView>
                ...
                <View style={styles.inputContainer}>
                    ...
                    <TextInput
                        ...
                        onChangeText={(t) => setNewQuestion({ ...newQuestion, correctAnswer: t })}
                        placeholder="Resposta Correta"
                    />
                </View>

                <Text style={[styles.inputLabel, {marginTop: 5}]}>Respostas Erradas</Text>
                <TextInput
                    style={[styles.textInput, {marginBottom: 8}]}
                    value={newQuestion.wrong1}
                    onChangeText={(t) => setNewQuestion({ ...newQuestion, wrong1: t })}
                    placeholder="Errada 1"
                />
                <TextInput
                    style={[styles.textInput, {marginBottom: 8}]}
                    value={newQuestion.wrong2}
                    onChangeText={(t) => setNewQuestion({ ...newQuestion, wrong2: t })}
                    placeholder="Errada 2"
                />
                <TextInput
                    style={[styles.textInput, {marginBottom: 20}]}
                    value={newQuestion.wrong3}
                    onChangeText={(t) => setNewQuestion({ ...newQuestion, wrong3: t })}
...
const styles = StyleSheet.create<any>({
  ...
  logoContent: { flexDirection: "row", marginTop: 60, justifyContent: 'center' },
  loginLogo: { width: 35, height: 35, marginRight: 10 },
  title: { fontFamily: "Rubik", fontSize: 25, fontWeight: "500" },
  headerBar: { width: "90%", flexDirection: "row", justifyContent: "space-between", marginTop: 25, alignItems: "center" },
  ...
  emptyState: { backgroundColor: "#FFF", borderRadius: 12, padding: 40, alignItems: "center", elevation: 3 },
  emptyStateText: { color: "#777", textAlign: "center" },
  questionCard: { backgroundColor: "#FFF", borderRadius: 12, padding: 20, marginBottom: 20, elevation: 3 },
  questionHeader: { flexDirection: "row", justifyContent: "space-between", marginBottom: 10, borderBottomWidth: 1, borderColor: '#eee', paddingBottom: 5 },
  questionCategory: { color: "#24bf94", fontWeight: "bold" },
  questionNumber: { color: "#777" },
  answerText: { fontSize: 16, color: "#333", backgroundColor: "#F8F8F8", padding: 10, borderRadius: 8 },
  inputContainer: { marginBottom: 15 },
  inputLabel: { fontWeight: "500", marginBottom: 8, color: "#333" },
  textInput: { backgroundColor: "#F8F8F8", borderWidth: 1, borderColor: "#E5E5E5", borderRadius: 8, padding: 12 },
  modalOverlay: { flex: 1, justifyContent: "center", alignItems: "center", backgroundColor: "rgba(0,0,0,0.5)" },
  modalContent: { width: "90%", height: "85%", backgroundColor: "white", borderRadius: 12, padding: 20 },
  modalTitle: { fontSize: 20, fontWeight: "bold", textAlign: "center", marginBottom: 20 },
  modalButtons: { flexDirection: "row", justifyContent: "space-between", marginTop: 10, marginBottom: 20 },
  cancelButton: { flex: 1, backgroundColor: "#ccc", padding: 15, borderRadius: 8, marginRight: 10, alignItems: "center" },
  confirmButton: { flex: 1, backgroundColor: "#24bf94", padding: 15, borderRadius: 8, marginLeft: 10, alignItems: "center" },
  cancelButtonText: { fontWeight: "bold" },
  confirmButtonText: { color: "white", fontWeight: "bold" },
  switchContainer: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 15, backgroundColor: '#f9f9f9', padding: 10, borderRadius: 8 },
  teamOption: { padding: 10, backgroundColor: '#eee', borderRadius: 20, marginRight: 10, height: 40, justifyContent: 'center' },
